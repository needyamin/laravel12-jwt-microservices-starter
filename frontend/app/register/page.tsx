...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      closeAlert();
      if (result.success) {
        await showSuccess('Account Created!', 'Welcome! Your account has been created successfully.');
        router.push('/');
      } else {
        setError(result.error || 'Registration failed');
        showError('Registration Failed', result.error || 'Could not create account. Please try again.');
      }
    } catch (err) {
      closeAlert();
